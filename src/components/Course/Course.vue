				...
				<!-- modal button -->
					...
					<div class="modal fade" id="buttonModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
									...
									<div class="modal-content bg-light">
										...
										<p class="text-dark">
										Anda Belum Login. Silahkan Login untuk join Course</p>
										</div>
	...
	export default {
		...
		axios.get(App.data().ListUrl.urlUser + EclassId).then(res=>{
			...
			axios.get( App.data().ListUrl.UrlCoursesByid + this.dataData).then(res=>{
				if(res.status == 200){
					...
					console.log('asolleee', res.data.result)
				}
			});
		...
		methods: {
			...
			joinCourse(id){
				...
				axios.post(App.data().ListUrl.UrlCoursesByid + id, {}, params).then(response => {
